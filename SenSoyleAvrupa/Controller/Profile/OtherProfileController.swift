...
extension OtherProfileController : UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        header.btnEditProfile.setTitle("Engelle veya bildir", for: .normal)
        header.btnEditProfile.addTarget(self, action: #selector(actionBlock), for: .touchUpInside)
    ...
    @objc func actionBlock() {
        print(id)
        print(email)
        print(CacheUser.email)
        let alert = UIAlertController(title: "Engelle", message: "Bu kullanıcıyı engellemek istediğinizden emin misiniz?", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Evet", style: .default, handler: { [self] (_) in
            ...
            AF.request("\(NetworkManager.url)/api/report", method: .post,parameters: parameters,encoding: URLEncoding.default)
