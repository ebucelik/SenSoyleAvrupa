...
open class VideoPicker: NSObject {
    ...
    public func present(from sourceView: UIView) {
        ...
        if let action = self.action(for: .camera, title: "Video çek") {
            alertController.addAction(action)
        }
        if let action = self.action(for: .savedPhotosAlbum, title: "Kamera rulosu") {
            alertController.addAction(action)
        }
        if let action = self.action(for: .photoLibrary, title: "Video kitaplığı") {
            alertController.addAction(action)
        }
        
        alertController.addAction(UIAlertAction(title: "İptal etmek", style: .cancel, handler: nil))
